// Frozen paired assessment from baseline ffacb6f782dc7b9772a4c31e924a303a4b2181d9. Evaluation only.
...
function assessmentModels(request: JevRouteRequest) {
...
export function buildBaselineAssessmentQuestions(
...
export function parseBaselineAssessmentDecision(
  raw: unknown,
  request: JevRouteRequest,
  latencyMs: number,
  parseChoice: (raw: unknown, keys: readonly string[], latency: number) => JevRouteResult,
): JevRouteResult {
  const questions = buildBaselineAssessmentQuestions(request)!;
