...
const makeNativeOperations = Effect.fn("PreviewManager.makeOperations")(function* (
  ...
  const sendRecordingController = Effect.fn("PreviewManager.sendRecordingController")(function* (
    tabId: string,
    state: PreviewTabState,
    controller: PreviewTabState["controller"],
    ...
    if (!capture?.consumers.has("recording")) return;
    yield* attempt({ operation: "recording.controller", tabId }, () => {
      const contents = webContents.fromId(webContentsId);
      if (contents && !contents.isDestroyed()) {
        contents.send(RECORDING_CONTROLLER_CHANNEL, controller, humanPoint);
      }
    }).pipe(Effect.ignore);
  });
  ...
  const update = Effect.fn("PreviewManager.update")(function* (
    ...
    if (Option.isSome(next)) {
      if (patch.controller !== undefined) {
        yield* sendRecordingController(tabId, next.value, patch.controller, humanPoint);
      }
  ...
  const clearAgentController = Effect.fn("PreviewManager.clearAgentController")(function* (
    ...
    if (Option.isSome(next)) {
      yield* sendRecordingController(tabId, next.value, "none");
      yield* emitIfCurrent(tabId, next.value);
    }
  });
  ...
  const jevBrowser = yield* makeJevBrowserController<PreviewManagerError>({
    ...
    expectAgentPointer: (tabId, point) => expectAgentInput(tabId, { kind: "pointer", ...point }),
    navigate,
    scroll: performAutomationScroll,
