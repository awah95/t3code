...
describe("PreviewManager", () => {
  ...
  effectIt.effect("activates semantic controls and reports reversible checked state", () =>
    withManager((manager) =>
      Effect.gen(function* () {
        const control = (
          id: string,
        ...
        const observation = (
          revision: string,
          controls: ReadonlyArray<ReturnType<typeof control>>,
        ) => ({
          revision,
        ...
        const observations = [
          observation("document-1:1", [
            control("button", "button"),
            control("checkbox", "checkbox"),
            control("combobox", "combobox"),
            control("native-select", "combobox", {
              tag: "select",
              value: "ts",
              options: [{ value: "js", label: "JavaScript", disabled: false }],
            }),
            control("menu", "menuitem", { name: "Options" }),
            control("menu-check", "menuitemcheckbox"),
            control("menu-radio", "menuitemradio"),
            control("option", "option"),
            control("radio", "radio"),
            control("switch", "switch"),
            control("tab", "tab"),
            control("allowed-link", "link", { href: "https://example.com/docs" }),
            control("outside-menu-link", "menuitem", { href: "https://outside.example/docs" }),
            control("disabled-tab", "tab", { disabled: true }),
          ]),
          observation("document-1:2", [
            control("menu", "menuitem", { name: "Options" }),
            control("toggle", "menuitemcheckbox", { name: "Match whole word" }),
          ]),
          observation("document-1:3", [
            control("menu", "menuitem", { name: "Options" }),
            control("toggle", "menuitemcheckbox", {
              name: "Match whole word",
              checked: true,
            }),
          ]),
          observation("document-1:4", [
            control("menu", "menuitem", { name: "Options" }),
            control("toggle", "menuitemcheckbox", { name: "Match whole word" }),
          ]),
          observation("document-1:4", [
        ...
        let notifyReadyStarted: (() => void) | undefined;
        ...
        const sendCommand = vi.fn(async (method: string, params?: Record<string, unknown>) => {
          ...
          if (method === "Runtime.evaluate") {
            ...
            if (expression.startsWith("globalThis.__t3JevBrowser?.guard(")) {
              return { result: { value: { ok: true, selector: "#target", x: 60, y: 35 } } };
            }
            ...
            return { result: { value: observations[observationIndex++] } };
          }
        ...
        fromId.mockReturnValue({
          ...
          isDestroyed: () => false,
          getType: () => "webview",
          getURL: () => "https://example.com/tools",
          getTitle: () => "Tool",
          isLoading: () => false,
          isDevToolsOpened: () => false,
          getZoomFactor: () => 1,
        ...
        yield* manager.createTab("tab_jev_controls");
        yield* manager.registerWebview("tab_jev_controls", 42);

        const observe = () =>
          manager.jevBrowserObserve("run-1", "tab_jev_controls", [], ["https://example.com"]);
        const activate = (current: JevAutomationObservation, targetId: string) => {
          const candidate = current.candidates.find(
            (entry) => entry.operation === "activate" && entry.targetId === targetId,
          );
          expect(candidate).toBeDefined();
          return manager.jevBrowserExecute("run-1", "tab_jev_controls", current.revision, {
        ...
        const unchanged = yield* observe();
        expect(unchanged.revision).toBe(disabled.revision);
        expect(evaluatedExpressions[0]).toMatch(/^\(function installJevBrowserObserver/);
        expect(evaluatedExpressions.at(-1)).toMatch(/^\(function installJevBrowserObserver/);
        expect(
        ...
        expect(yield* activate(unchanged, "menu")).toEqual({ status: "executed" });
        holdReady = true;
        const pendingObservation = yield* observe().pipe(
          Effect.forkChild({ startImmediately: true }),
        );
        yield* Effect.promise(() => readyStarted);
        expect(yield* manager.jevBrowserCancel("run-1")).toBe(true);
        releaseReady?.();
        const pendingExit = yield* Fiber.await(pendingObservation);
        expect(Exit.isFailure(pendingExit)).toBe(true);
        expect(yield* manager.jevBrowserCancel("run-1")).toBe(false);
      }),
    ),
  );
