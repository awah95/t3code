...
Pass \`-w\` or \`--worktree \[name\]\` to run the agent in a new Git worktree instead of editing your current checkout directly. Cursor creates these checkouts under \`~/.cursor/worktrees//\`, alongside worktrees created from the editor. If you omit \`name\`, Cursor generates one.
...
Combine \`--workspace \` when you need an explicit repository root. Otherwise the CLI uses the current working directory. \`--worktree\` only changes where the agent makes file edits inside that project.
