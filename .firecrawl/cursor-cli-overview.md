...
Configure command execution settings with \`/sandbox\` or the \`--sandbox \` flag (\`enabled\` or \`disabled\`). Toggle sandbox mode on or off and control network access through an interactive menu. Settings persist across sessions.
